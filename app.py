...
def edit_pet(pet_id):
    if request.method == "POST":
        ...
        mongo.db.pets.update_one({"_id": ObjectId(pet_id)}, {"$set": update_data})  # noqa
...
def delete_pet(pet_id):
    pet = mongo.db.pets.find_one({"_id": ObjectId(pet_id)})
    # the session["user"] must be the pet owner
    ...
    flash(f"Farewell {pet["pet_name"]} - Sadly Deleted!")
...
def breed_details(breed_name):
    """
    filter all pets by selected breed from the database
    """
    ...
    all_pets = list(mongo.db.pets.find({"pet_breed": breed_name}))
    if "user" in session:
        user = mongo.db.users.find_one({"username": session["user"]})
        liked_pets = user.get("liked_pets", [])
    else:
...
def like_pet(pet_id):
    ...
    flash(f"Lovely to hear you like {pet["pet_name"]}!")
...
def unlike_pet(pet_id):
    ...
    flash(f"How sad, you no longer like {pet["pet_name"]}.")
    return redirect(request.referrer or url_for("get_pets"))
